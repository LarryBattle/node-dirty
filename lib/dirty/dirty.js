...
var Dirty = exports.Dirty = module.exports = function (path) {
	...
	this._keys = [];
	this._queue = [];
	//fix for https://github.com/felixge/node-dirty/issues/26
...
// key must be defined - fix for https://github.com/felixge/node-dirty/issues/30
Dirty.prototype.set = function (key, val, cb) {
	...
	if (val === undefined) {
		...
		// Fixed to save the source of functions.
...
Dirty.prototype.setupReadStream = function () {
	...
	this._readStream.on('end', function () {
		if (this.readBuffer.length) {
			self.emit('error', new Error('Corrupted row at the end of the db: ' + this.readBuffer));
		}
...
Dirty.prototype.getKeyValueAsString = function (key) {
	return this.convertRowToString(this.getKeyValueObject());
};
